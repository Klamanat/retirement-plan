...
export default function TripsPage() {
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-5 lg:space-y-6 max-w-5xl">
      ...
      {/* Header */}
      <div className="relative bg-gradient-to-br from-violet-600 via-violet-700 to-purple-700 rounded-2xl px-6 py-5 text-white overflow-hidden shadow-lg shadow-violet-500/20">
        ...
        <div className="relative flex items-center justify-between gap-4">
          <div>
            <div className="inline-flex items-center gap-1.5 mb-2">
              ...
              <span className="text-violet-200 text-xs font-medium tracking-wide">แผนเดินทาง</span>
            </div>
            <h1 className="text-xl font-bold tracking-tight">จัดการทริปและงบประมาณ</h1>
          </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 lg:gap-4">
        {[
          { label: "ทริปทั้งหมด", value: stats.total, unit: "ทริป", gradient: "from-indigo-500 to-violet-600", shadow: "shadow-indigo-500/25" },
          { label: "งบประมาณรวม", value: fmt(stats.budget), unit: "฿", gradient: "from-violet-500 to-purple-600", shadow: "shadow-violet-500/25" },
          { label: "ใช้จ่ายไปแล้ว", value: fmt(stats.spent), unit: "฿", gradient: "from-rose-500 to-pink-600", shadow: "shadow-rose-500/25" },
          { label: "เสร็จแล้ว", value: stats.completed, unit: "ทริป", gradient: "from-emerald-500 to-teal-600", shadow: "shadow-emerald-500/25" },
        ].map(c => (
          <div key={c.label} className={`relative rounded-2xl p-5 bg-gradient-to-br ${c.gradient} text-white shadow-lg ${c.shadow} hover:shadow-xl hover:-translate-y-0.5 transition-all overflow-hidden`}>
            <div className="absolute -top-4 -right-4 w-20 h-20 bg-white/10 rounded-full blur-xl pointer-events-none" />
            <div className="relative">
              <p className="text-xs font-medium text-white/70 uppercase tracking-wide">{c.label}</p>
              <p className="text-2xl font-bold text-white mt-1">{c.value}</p>
              <p className="text-xs text-white/60 mt-0.5">{c.unit}</p>
            </div>
          </div>
        ))}
      </div>
            ...
            return (
              <div key={trip.id}
                ...
                <div className={`h-[3px] w-full bg-gradient-to-r ${
                  trip.status === "planning"  ? "from-indigo-500 to-violet-500" :
                  trip.status === "booked"    ? "from-amber-400 to-orange-500" :
                  trip.status === "completed" ? "from-emerald-400 to-teal-500" :
